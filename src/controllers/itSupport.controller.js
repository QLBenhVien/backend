...
const NhanVienFactory = require("../patterns/admin/NhanVienFactory");
...
const createNhanVien = async (req, res) => {
  try {
    const { role, ...nhanVienData } = req.body;
    // Sử dụng Factory Method
    const { savedNhanVien, savedAccount } = await NhanVienFactory.create(
      role,
      nhanVienData
    );
    return successResponse(req, res, {
    ...
    // end
    // //code cũ
    // const { email, password, HoTen, DiaChi, GioiTinh, SDT, MaCV, Khoa, role } =
    //   req.body;
...
const disableNhanVien = async (req, res) => {
  try {
    ...
    const nhanVien = await TaiKhoan.findOneAndUpdate(
      { _id: MaTK },
      { active: false }
    );
...
const disableBenhNhan = async (req, res) => {
  try {
    ...
    const benhnhan = await TaiKhoan.findOneAndUpdate(
      { _id: accountId },
      { active: false }
    );
    console.log("benhnhan:", benhnhan);
