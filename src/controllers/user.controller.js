...
module.exports.Resetpassword = async (req, res, next) => {
  ...
  try {
    ...
    const Email = await TaiKhoan.findOne({ email });
        ...
        const updatedTK = await TaiKhoan.findOneAndUpdate({ email: email }, { password: hashedPassword }, { new: true, runValidators: true });
...
module.exports.xemchitietLichKham = async (req, res) => {
    try {
        ...
        if (!doctor) {
            return res.status(404).json({ message: "Không tìm thấy bác sĩ với thông tin đã cho." });
        }
...
module.exports.timKiemNhanVien = async (req, res) => {
    try {
        ...
        if (!HoTen && !Tenkhoa) {
            return res.status(400).json({
                message: "Phải cung cấp ít nhất một trong các tham số: HoTen hoặc Tenkhoa.",
            });
        }
