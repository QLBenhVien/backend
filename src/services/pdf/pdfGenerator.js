...
const generateAndUploadPDF = async (data, option) => {
  if (option === "phieukham") {
  ...
  } else if (option === "bienlai") {
    const inputPath = path.resolve(__dirname, "../files/bienlai.pdf");
    const outputPath = path.resolve(
      __dirname,
      `../files/bienlai-${data.tenbenhnhan}.pdf`
    );
    ...
    const replaceText = async () => {
      ...
      await replacer.addString("khoa", String(data.khoa || ""));
      await replacer.addString("bacsi", String(data.tenbacsi || ""));
      await replacer.addString("noidung", String(data.noidung || ""));
      await replacer.addString("taikham", String(data.taikham || ""));
      await replacer.addString("tiendichvu", String(data.tiendichvu || ""));
      await replacer.addString("tongtien", String("150.000 VND" || ""));
